...
public class Application {
    ...
    private boolean optionProcessor(String input) throws CalculatorException {
        Options option;
        try {
            option = Options.valueOf(input.toUpperCase().substring(1));
    ...
    private boolean processLine(String line) throws CalculatorException {
        line = line.trim();
        if (line.contains("=")) {
        ...
        if (strings.size() == 1) {
            ...
            if (isParsable(s)) {
                ...
                throw new CalculatorException("Unknown variable : " + line);
            }
        }
        Solver solver = new Solver(line, variableMap);
        solver.execute();
        return true;
    }
    ...
    private void processAssignments(String line) {
        ...
        if (strings.size() > 2 || (!isParsable(variable) && !isVariable(variable))) {
            throw new CalculatorException("Invalid assignment for : " + line);
        }
        ...
        if (!isVariable(variable) && !isParsable(variable)) {
            throw new CalculatorException("Unknown variable : " + variable);
        }
        BigInteger a;
